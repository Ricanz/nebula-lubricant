
        <header class="main-header">
            <div class="main-header__wrapper container">
                <div class="main-header__wrapper-inner">
                    <div class="main-header__logo">
                        <a href="{{ route('home') }}" class="text_logo">
                            <p>Nebula Enviro</p>
                        </a>
                    </div>
                    <div class="main-header__menu-box">
                        <div class="main-header__menu-box-bottom">
                            <nav class="main-menu">
                                <div class="main-menu__wrapper">
                                    <div class="main-menu__wrapper-inner">
                                        <div class="main-menu__main-menu-box">
                                            <a href="#" class="mobile-nav__toggler"><i class="fa fa-bars"></i></a>
                                            <ul class="main-menu__list">
                                                <li>
                                                    {{-- <a href="{{ url('/about') }}" class="text-white">Profile</a> --}}
                                                    <a href="/#profile" class="text-white">Profile</a>
                                                </li>
                                                <li>
                                                    {{-- <a href="{{ url('/article-detail/tentang-kami') }}" class="text-white">Our Service</a> --}}
                                                    <a href="/#our_service" class="text-white">Our Service</a>
                                                </li>
                                                <li>
                                                    {{-- <a href="{{ url('/article') }}" class="text-white">Blog</a> --}}
                                                    <a href="/#blog" class="text-white">Blog</a>
                                                </li>
                                                <li>
                                                    <a href="/#contact_us" class="text-white">Contact Us</a>
                                                </li>
                                                <li>
                                                    {{-- <a href="https://api.whatsapp.com/send?phone=6281288889662" target="_BLANK" class="btn btn-warning-subs px-4">
                                                        Konsultasi Gratis
                                                    </a> --}}
                                                    <a href="/#konsultasi_gratis" class="btn btn-warning-subs px-4">
                                                        Konsultasi Gratis
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </nav>
                        </div>
                    </div>
                </div>
            </div>
        </header>

        <div class="stricky-header stricked-menu main-menu">
            <div class="sticky-header__content"></div>
        <!-- /.sticky-header__content -->
        </div>
        <!-- /.stricky-header -->